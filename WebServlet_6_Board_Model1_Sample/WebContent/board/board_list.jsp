...
<!-- 	
	...
	-include
	다른 페이지의 결과를 가져와 출력
	포함할 jsp 페이지의 실행 결과를 현재 위치에 포함시키는 역할
	-forwarding
	하나의 jsp페이지에서 다른 jsp 페이지로
	요청 처리를 전달할때 사용
	다른 페이지로 이동해서 작업수행후에 결과를 가져와서
	현재 페이지에 출력
	 -->
	...
	<%
       ...
       //102건   pagesize 5   pagecount : 21 페이지 
							...
							<select name="ps" onchange="submit()">
								<%
				   					/* 
				   					    ...
				   					    Url 에 주소값으로 요청  ^^^^^^^^^^^^^^^^^^^^ 
				   					        현재 주소창에 써있는 주속가 action 의 주소값입니다 
