...
public class boarddao {
	...
	public List<board> list(int cpage, int pagesize) {
		 ...
		 * DESC , step ASC ) ) WHERE rn BETWEEN ? AND ?;
		 * 
		 * -------------------------------------------------------------------- select *
		...
		try {
			...
			String sql = "select * from "
					...
					+ " jspboard ORDER BY refer DESC , step ASC " + " ) where rownum <= ? " + // 5
					") where rn >= ?"; // 1
	...
	public board getContent(int idx) {
		...
		try {
			...
			if (rs.next()) {
				...
				boarddto = new board(idx, writer, pwd, subject, content, writedate, readnum, filename, filesize,
						homepage, email, refer, depth, step);
			}
